...
pub(crate) fn new_remote_github_repository() -> anyhow::Result<()> {
    ...
    let github_owner_or_organization = cargo_toml
        .github_owner().context("{RED}ERROR: Element Repository in Cargo.toml does not contain the github_owner!{RESET}")?;
    ...
    if !git_has_remote()? {
        let description = cargo_toml
            .package_description().context("{RED}ERROR: Element Description in Cargo.toml does not exist!{RESET}")?;
        ...
        println!("  {YELLOW}name: {}{RESET}", json_value.get("name").context("json_value name is None")?.as_str().context("json_value name is None")?);
        println!(
            "  {YELLOW}description: {}{RESET}",
            json_value.get("description").context("json_value description is None")?.as_str().context("json_value description is None")?
        );
        let repo_html_url = json_value.get("html_url").context("json_value html_url is None")?.as_str().context("json_value html_url is None")?.to_string();
...
pub(crate) fn description_and_topics_to_github() ->anyhow::Result<()>{
    ...
    if is_old_metadata_different {
        ...
        let gh_description = json.get("description").context("description is None")?.as_str().context("description is None")?;
        let gh_topics = json.get("topics").context("topics is None")?.as_array().context("topics is None")?;
        let gh_topics: Vec<String> = gh_topics.iter().map(|value| value.as_str().expect("map(||) cannot use ? propagation").to_string()).collect();
...
pub(crate) fn github_api_upload_asset_to_release(github_owner_or_organization: &str, repo: &str, release_id: &str, path_to_file: &str) ->anyhow::Result<()>{
    println!("  {YELLOW}Uploading file to GitHub release: {path_to_file}{RESET}");
    let file = camino::Utf8Path::new(&path_to_file);
    let file_name = file.file_name().context("file_name is None")?;
    ...
    rt.block_on(async move {
        let file = tokio::fs::File::open(file).await.expect("Propagation ? cannot be used in async block");
